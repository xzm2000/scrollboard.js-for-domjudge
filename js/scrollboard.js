...
function getSubmitList(tt) {
    ...
    $.ajax({
        ...
        success: function(result) {
            for (var i in result) {
				var sub = result[i];
				var ss="t"+sub.team_id;
				while (ss!=ss.replace('.','_')) ss=ss.replace('.','_');
				for (var j in sub.problems){
					var nowproblem = sub.problems[j];
					if (nowproblem.solved == false){
						for (var k = 1; k <= nowproblem.num_judged; k++)
							data.push(new Submit(1, ss, nowproblem.label, tt.getTime() - 60000, 1));
						for (var k = 1; k <= nowproblem.num_pending; k++)
							data.push(new Submit(1, ss, nowproblem.label, tt, -1)); 
					}
					else{
						for (var k = 1; k < nowproblem.num_judged; k++)
							data.push(new Submit(1, ss, nowproblem.label, tt.getTime() - 60000, 1));
						data.push(new Submit(1, ss, nowproblem.label, tt.getTime() + 60000 * nowproblem.time, 0)); 
					}
				}
            }
    ...
    $.ajax({
        ...
        success: function(result) {
            for (var key in result) {
				...
				var sss = team.name;
				while (ss!=ss.replace('.','_')) ss=ss.replace('.','_');
				while (sss!=sss.replace('.','_')) sss=sss.replace('.','_');
				data[ss] = new Team(ss, sss , null, true);
				if (sss == "mcislab") data[ss].official = false;
...
Team.prototype.init = function(board) {
    ...
    for (var key in this.submitList) {
        ...
        if (p.isAccepted) {
			...
			if (sub.acceptedTime > this.lastAC)
				this.lastAC=sub.acceptedTime;
